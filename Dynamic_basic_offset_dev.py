...
def create_dag(dag_id, schedule, default_args, config):
    ...
    def fetch_data(ti,**kwargs) -> None:
        ...
        print(url)
        if payload:
            res = requests.request(rqt, url, headers=headers, json=payload, auth=HTTPBasicAuth(username, password)).json()
            res2 = requests.request(rqt, url, headers=headers, json=payload, auth=HTTPBasicAuth(username, password))
        else:
            res = requests.request(rqt, url, headers=headers, auth=HTTPBasicAuth(username, password)).json()
            res2 = requests.request(rqt, url, headers=headers, auth=HTTPBasicAuth(username, password))
        json_response = res
        ...
        for x in range (0, totalIterations):
            if currentIterations <= totalIterations:
                if pagination:
                    ...
                    if payload:
                        response = requests.request(rqt, newUrl, headers=headers, json=payload, auth=HTTPBasicAuth(username, password)).json()
                    else:
                        response = requests.request(rqt, newUrl, headers=headers, auth=HTTPBasicAuth(username, password)).json()
                    currentIterations+=1
                    ...
                    if payload:
                        response = requests.request(rqt, url2, headers=headers, json=payload, auth=HTTPBasicAuth(username, password)).json()
                    else:
                        response = requests.request(rqt, url2, headers=headers, auth=HTTPBasicAuth(username, password)).json()
...
countUrl = vinylDomUrl+"/basicoffset?$count=true&$limit=0"
headers = {
    'Content-Type': 'application/json',
    'X-API-Key': vinylApiKey
}
...
limit = 100
url = vinylDomUrl+"/basicoffset?$count=true&$limit="+str(limit)
totalIterations = math.ceil(int(count) / int(limit))
currentIterations = 0
newOffset = 0
for x in range (0, totalIterations):
...
with DAG(
    dag_id = 'Dynamic_basic_offset_dev',
