...
def create_dag(dag_id, schedule, default_args, config):
    ...
    #Billing  2242e16b-85fd-4a01-9bec-b5d2d2d2bc91
    #Operational  525be0fe-374c-4332-9e1f-2162b601c190
    ...
    def fetch_data(ti) -> None:
        ...
        newRecLim = recLim / 100 
        pagination = ti.xcom_pull(key="nextLink", task_ids="TokenOffsetTest")
        print(pagination)
        dt = now.strftime("%d-%m-%Y %H:%M:%S")
        count = 0
        staging = list()
        if pagination:
            pagUrlLoc = pagination.split(".")
            ...
            while more == 1:
                ...
                if url == "None" or url == None or url == "":
                    break
                    #print("URL IS NOW NONE")
                    
                print("START URL before call ----->", url)
                if payload:
                    json_response = requests.request(rqt, url, headers=headers, json=payload, auth=HTTPBasicAuth(username, password)).json()
                    json_response2 = requests.request(rqt, url, auth=HTTPBasicAuth(username, password), headers=headers).json()
                else:
                    json_response = requests.request(rqt, url, auth=HTTPBasicAuth(username, password), headers=headers).json()
                    json_response2 = requests.request(rqt, url, auth=HTTPBasicAuth(username, password), headers=headers)
                
                if json_response2.status_code != 200:
                    raise TypeError("response from API ==> ", json_response)
                
                #print("Response after call ----->", json_response)
                ...
                #print(staging, type(staging))
                staging.append(json.dumps(json_response))
                #print(staging, type(staging))
                ...
                if (count/int(newRecLim)).is_integer() is True:
                    with open(local_path, 'w') as f:
                        ...
                        print('file Written')
                        f.close()
                        s3.Bucket(s3BucketData).upload_file(Filename="TEST_API_DATA.json", Key= ApiResFilePath+ "data_"+str(count)+"_.json")  
                    staging = []
                ...
                try:
                    ...
                    nextUrl = my_result.getvalue()
                except:
                    print("Stopping here")                
                    nextUrl = None
                    more +=1
                    print("Print nextUrl---->", nextUrl)

                print("current response -->", json.dumps(json_response))
                print("nextURL =>", type(nextUrl), nextUrl) 

                    
                if nextUrl:
                    print("NEXT URL IN IF ------>", nextUrl)
                    url = nextUrl.strip()
                    #print(url)
                    #print(json.dumps(json_response))
            ...
            if payload:
                ...
                json_response = requests.request(rqt, url, headers=headers, auth=HTTPBasicAuth(username, password)).json()
...
countUrl = vinylDomUrl+"/basicurl?$count=true&$limit=0"
headers = {
    'Content-Type': 'application/json',
    'X-API-Key': vinylApiKey
}
...
limit = 100
url = vinylDomUrl+"/basicurl?$count=true&$limit="+str(limit)
...
#print(res['items'])
#give to func below using config param 
#

with DAG(
    dag_id = 'Dynamic_basic_url_dev',
