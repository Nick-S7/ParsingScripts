...
def create_dag(dag_id, schedule, default_args, config):
    ...
    def objects_1_2_snowFlake(ti):
        ...
        if destinationType == 'Tableau':
            ...
            finaldf = finaldf.replace({True: 'true', False:'false'})
            #print(finaldf)
            ...
            for obj2 in s3.Bucket(s3BucketData).objects.filter(Prefix=newPath):
                ...
                if (count_row/100000).is_integer() is True:
                    ...
                    with engine.connect().execution_options(autocommit=True) as con:
                        ...
                        try:
                            ...
                            print("after write check ---->", df_check.head(10))
                            log_count_after_write(endpointID, count_row, table_name, schema, database)
                        except:
                    ...
                    df = pd.DataFrame()
                    #tempdf = pd.DataFrame()
                    #print("finaldf after clear ---->", finaldf)
                    #print("df after clear ---->", df)
            if (count_row/100000).is_integer() is False:
                finaldf.columns = map(str.upper, finaldf.columns)
                for z in finaldf.columns:
                    finaldf = finaldf.rename(columns={z: z.replace(".", "_")})
                    finaldf = finaldf.rename(columns={z: z.replace("/", "_")})    
                ...
                with engine.connect().execution_options(autocommit=True) as con:
                    ...
                    try:
                        ...
                        print('Issue writing to Snowflake:', error)
                listy=[]
                finaldf= pd.DataFrame()
                df = pd.DataFrame()
                #tempdf = pd.DataFrame()
...
with DAG(
    dag_id = 'Dynamic_noAuth_staticLimit',
