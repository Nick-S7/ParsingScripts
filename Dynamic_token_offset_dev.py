...
def create_dag(dag_id, schedule, default_args, config):
            ...
            if type(v) is str:
                ...
                for kk, vv in v.items():
                    if vv:
                        ...
                        continue
            elif type(v) is list and k == "payload":
                for d in v:
                    param[d['key']] = d['value']
                ti.xcom_push(key=k, value=param)
            elif type(v) is list:
    ...
    def fetch_data(ti) -> None:
        ...
        access_token = ti.xcom_pull(key="accessToken", task_ids="TokenOffsetTest")
        print(access_token)
        if configHeaders:
            headers = {'Authorization' : 'Bearer ' + access_token} | configHeaders
        else:
        ...
        if params:
            ...
            for k, v in params.items():
                if count == 0:
                    ...
                    paramG =  paramG +"&"+ str(k)+"="+str(v) 
            url = dataUrl+ "/"+ endpoint + "?"+paramG
        else:
        ...
        if payload:
            ...
            payColumn = payload['Column']
            payTable =  payload['Table']
            payKey =  payload['PayloadKey']
            
            with engine.connect().execution_options(autocommit=True) as con:

                payQuery = "SELECT " + payColumn + " FROM " + payTable
                payRes = pd.read_sql(payQuery, engine)
                print("query------>", payQuery)
                #print("responseType------>", type(payRes))
                #print("response------>", payRes)
                if isinstance(payRes, pd.DataFrame):
                    resList = payRes.values.tolist()
                    tmpPayload = list()
                    for el in resList:
                        hold = dict()
                        for eid in el:
                            hold[payKey] = eid
                            tmpPayload.append(hold)
                    #print(newPayload)
                else:
                    print("NOT A DF")
            newPayload = dict()
            newPayload["requests"] = tmpPayload
            #print(newPayload)

            res = requests.request(rqt, url, headers=headers, json=newPayload).json()
        else:
            res = requests.request(rqt, url, headers=headers).json()
        json_response = res
        print(json_response)
        
        pagination = ti.xcom_pull(key="offsetPagination", task_ids="TokenOffsetTest")
        #print(pagination)
        staging=list()
        if pagination:
            #pagSplitLimit = pagination['limit'].replace(".", "']['")
            pagSplitLimit = pagination['limit'].split(".")
            pagSplitCount = pagination['count'].split(".")
            #tcount = str("count")
            #print(json_response[tcount])
            #limit = str()
            i = 0
            out = {}
            for x in pagSplitLimit:
                i += 1
                out[i] = [x]
            start = "json_response"
            end = str()
            ...
            expression = 'print('+final+')' # any exec() expression here
            exec(expression)
            # Step 6: Clean up by redirecting stdout to Python shell
            sys.stdout = tmp
            # Step 7: Get and print the string from stdout
            limit = my_result.getvalue()
            e = 0
            out2 = {}
            for x in pagSplitCount:
                e += 1
                out2[e] = [x]
            start2 = "json_response"
            end2 = str()
            ...
            my_result2 = StringIO()
            # Step 4: Assign Standard Output Stream to StringIO object
            sys.stdout = my_result2
            # Step 5: Print to the standard output
            expression2 = 'print('+final2+')' # any exec() expression here
            exec(expression2)
            # Step 6: Clean up by redirecting stdout to Python shell
            sys.stdout = tmp2
            # Step 7: Get and print the string from stdout
            count = my_result2.getvalue()
        ...
        for x in range (0, totalIterations):
            if currentIterations <= totalIterations:
                if pagination:
                    if params:
                        newUrl = url2 + "&"+ offsetType +"=" + str(newOffset)
                    else:
                        newUrl = url2 + "?"+ offsetType +"=" + str(newOffset)
                    print(url2)
                    newUrl = url2 + "&"+ offsetType +"=" + str(newOffset)
                    if payload:
                        response = requests.request(rqt, newUrl, headers=headers, json=newPayload).json()
                    else:
                    ...
                    if params:
                        url2 = url2 + "&"+ offsetType +"=" + str(newOffset)
                    else:
                        url2 = url2 + "?"+ offsetType +"=" + str(newOffset)
                                            
                    if payload:
                        response = requests.request(rqt, url2, headers=headers, json=newPayload).json()
                    else:
                        response = requests.request(rqt, url2, headers=headers).json()
                                                        ...
                                                        if type(c) is str:
                                                            ...
                                                            for d in c:
                                                                ...
                                                                if type(c[d]) is list:
                                                                    ...
                                                                    for e in c[d]:
                                                                        if type(e) is dict:
                                                                            listdex7 = str(uuid.uuid4())
                                                                            o3 = {endpoint+"_"+y+"_"+a+"_"+b+"_"+d+"_GID": listdex7, "TablePrefix": endpoint+"_"+y+"_"+a+"_"+b+"_"+d, endpoint+"_"+y+"_"+a+"_"+b+"_GID": listdex6}
                                                                            e = {**o3, **e}
                                                                            listed7.append(e)
                                                                        c[d] = listed7
                                                                        print("e------->", e)
                                                                ...
                                                                if newKey == k and newKey in listCount: 
                                                                    ...
                                                                    if (countObj[newKey]/int(recLim)).is_integer() is True:
                                                                        ...
                                                                        with open(local_path, 'w') as f:
                                                                            ...
                                                                            print(newPath+y+"/"+a+"/"+file)
                                                                            f.close()
                                                                            s3.Bucket(s3BucketData).upload_file(Filename="l4_list_w_dict_DATA.json", Key= newPath+newKey+file+".json")
                                                                        #print(listtos3[newKey])
                                                                        listtos3[newKey] = []
                                                                elif newKey != k and newKey not in listCount:
                                                                    #print("if k DOES NOT = a")
        ...
        for k,v in listtos3.items():
            ...
            if (countObj[k]/int(recLim)).is_integer() is False:
                ...
                with open(local_path, 'w') as f:
                    ...
                    print('remainder of files Written')
                    f.close()
                    s3.Bucket(s3BucketData).upload_file(Filename="l4_list_w_dict_DATA.json", Key= newPath+k+file+".json")
            else:
                print("nothing to write to s3")
                continue
    ...
    def arrays_5f(ti):
        ...
        endpoint = endpoint.replace("/", "_")
        local_path=os.path.join(os.getcwd(), 'l5f_list_w_dict_DATA.json')
        newPath = tagCustomerID +"/"+ schema +"/" + endpoint + "/" + str(dt) +"/"
        path = tagCustomerID +"/"+ schema +"/" + endpoint + "/" + str(dt) +"/od_keys/"
                                                                            ...
                                                                            if newKey == k and newKey in listCount: 
                                                                                ...
                                                                                if (countObj[newKey]/int(recLim)).is_integer() is True:
                                                                                    ...
                                                                                    with open(local_path, 'w') as f:
                                                                                        ...
                                                                                        print('file Written') ##########FILE KEEP RESETTING At 10 so it gets over written in s3, need different count for files.
                                                                                        f.close()
                                                                                        s3.Bucket(s3BucketData).upload_file(Filename="l5f_list_w_dict_DATA.json", Key= newPath+newKey+file+".json")
                                                                                    #print(listtos3[newKey])
                                                                                    listtos3[newKey] = []
                                                                            elif newKey != k and newKey not in listCount:
                                                                                #print("if k DOES NOT = a")
        ...
        for k,v in listtos3.items():
            if (countObj[k]/int(recLim)).is_integer() is False:
                ...
                with open(local_path, 'w') as f:
                    ...
                    print('remainder of files Written')
                    f.close()
                    s3.Bucket(s3BucketData).upload_file(Filename="l5f_list_w_dict_DATA.json", Key= newPath+k+file+".json")
            else:
                print("nothing to write to s3")
                continue                        

    def arrays_5f_snowFlake(ti):
        dt = ti.xcom_pull(key="dt", task_ids="Fetch_Data")
        database = ti.xcom_pull(key="database", task_ids="TokenOffsetTest")
        endpoint = ti.xcom_pull(key="endpoint", task_ids="TokenOffsetTest")
        customEndpointName = ti.xcom_pull(key="customEndpointName", task_ids="TokenOffsetTest")
    ...
    def log_endpoint_complete(ti, **kwargs):
        ...
        url = "https://app.getourdata.com/rest/v1/getourdata/endpointlog"
    ...
    with DAG(dag_id,
        ...
        Arrays_5f = PythonOperator(
            task_id='Arrays_5f',
            python_callable=arrays_5f,
            retries=3,
            retry_delay=5,
            retry_exponential_backoff=True,
        )

        Arrays_5f_Snowflake = PythonOperator(
            task_id='Arrays_5f_Snowflake',
            python_callable=arrays_5f_snowFlake,
            retries=3,
            retry_delay=5,
            retry_exponential_backoff=True,
        )
        ...
        TokenOffsetTest >> Fetch_Data >> KeyRelationshipS >> Objects_1_2 >> Objects_1_2_SnowFlake >> Arrays_1 >> Arrays_1_SnowFlake >> Arrays_2 >> Arrays_2_SnowFlake >> Arrays_3 >> Arrays_3_SnowFlake >> Arrays_4 >> Arrays_4_SnowFlake >> Arrays_5f >> Arrays_5f_Snowflake >> Log_Endpoint_Complete
...
countUrl = vinylDomUrl+"/tokenoffset?$count=true&$limit=0"
headers = {
    'Content-Type': 'application/json',
    'X-API-Key': vinylApiKey
}
...
limit = 100
url = vinylDomUrl+"/tokenoffset?$count=true&$limit="+str(limit)
totalIterations = math.ceil(int(count) / int(limit))
currentIterations = 0
newOffset = 0
for x in range (0, totalIterations):
    if currentIterations <= totalIterations:
        ...
        #    else:
        #        print("ENV VAR IS DEV ----- Files not written if not local")
            globals()[dag_id] = create_dag(dag_id, schedule, default_args, config)
        currentIterations+=1
        newOffset = currentIterations * int(limit)
        
with DAG(
    dag_id = 'Dynamic_token_offset_dev',
