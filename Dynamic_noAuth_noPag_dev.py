...
def create_dag(dag_id, schedule, default_args, config):
    ...
    def fetch_data(ti,**kwargs) -> None:
        ...
        with open(local_path, 'w') as f:
            ...
            print('file Written')
            f.close()
            s3.Bucket(s3BucketData).upload_file(Filename="TEST_API_DATA.json", Key= ApiResFilePath+ "data_1_.json")     
        print("FILE PATH --->", ApiResFilePath)
    ...
    def keyrelationships(ti):
        ...
        for obj2 in s3.Bucket(s3BucketData).objects.filter(Prefix=path):         
            ...
            if type(resx) is list:
                print("RESX IS LIST")

                for every in resx:
                    #if type(every) is str:
                    #    for ea in resx[every]:
                    print(count)
                    count +=1
                    print("every --->", every)
                    print("resx[every] --->", resx[every])
        ...
        for obj2 in s3.Bucket(s3BucketData).objects.filter(Prefix=path):
            ...
            for d in all:
                ...
                if (count/int(recLim)).is_integer() is True:
                    ...
                    print("before --->", listtos3)
                    with open(local_path, 'w') as f:
                        f.write(json.dumps(listtos3))
                        print('file Written1') ##########FILE KEEP RESETTING At 10 so it gets over written in s3, need different count for files.
                        f.close()
                        s3.Bucket(s3BucketData).upload_file(Filename="lvl1dictDATA.json", Key= newPath+str(count)+".json")
                    listtos3[endpoint] = []
                    print("after --->", listtos3)
                    listy=[]
        if (count/int(recLim)).is_integer() is False:
            listtos3[endpoint] = listy
            with open(local_path, 'w') as f:
                f.write(json.dumps(listtos3))
                print('file Written2') ##########FILE KEEP RESETTING At 10 so it gets over written in s3, need different count for files.
                f.close()
                s3.Bucket(s3BucketData).upload_file(Filename="lvl1dictDATA.json", Key= newPath+str(count)+".json")
            listtos3[endpoint] = []
        #print(listtos3)
...
countUrl = vinylDomUrl+"/noauthnopag?$count=true&$limit=0"
headers = {
    'Content-Type': 'application/json',
    'X-API-Key': vinylApiKey
}
...
limit = 100
url = vinylDomUrl+"/noauthnopag?$count=true&$limit="+str(limit)
...
with DAG(
    dag_id = 'Dynamic_noAuth_noPag_dev',
