    ...
    def fetch_data(ti) -> None:
        ...
        if pagination:
            ...
            while more == 1:
                ...
                if payload:
                    json_response = requests.request(rqt, url, headers=headers, json=payload).json()
                    json_response2 = requests.request(rqt, url, headers=headers).json()
                else:
                    json_response = requests.request(rqt, url, headers=headers).json()
                    json_response2 = requests.request(rqt, url, headers=headers)
...
countUrl = vinylDomUrl+"/nourl?$count=true&$limit=0"
headers = {
    'Content-Type': 'application/json',
    'X-API-Key': vinylApiKey
}
...
limit = 100
url = vinylDomUrl+"/nourl?$count=true&$limit="+str(limit)
