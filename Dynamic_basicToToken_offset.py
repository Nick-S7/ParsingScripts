...
def create_dag(dag_id, schedule, default_args, config):
    ...
    def log_endpoint_no_data(ti, **kwargs):
        ...
        payload = json.dumps({
          ...
          "log": "/// No Data Returned From API /// 404 Not Clean Data ///",
          "runID": run_id,
    ...
    def fetch_token(ti) -> None:
        #pull ti 
        authUrl = ti.xcom_pull(key='authUrl', task_ids='TokenOffsetTest')
        username = ti.xcom_pull(key='username', task_ids='TokenOffsetTest')
        password = ti.xcom_pull(key='password', task_ids='TokenOffsetTest')
        print(authUrl, username, password)
        res = requests.request("GET", authUrl, auth=HTTPBasicAuth(username, password))
    ...
    def fetch_data(ti, **kwargs) -> None:
        ...
        access_token = ti.xcom_pull(key="token", task_ids="Fetch_Token")
        print(access_token)
        if configHeaders:
        ...
        print("rqt----->", rqt)
        goodStatusCodes = [200, 202]

        if payload:
            res2 = requests.request(rqt, url, headers=headers, json=payload)
        else:
            res2 = requests.request(rqt, url, headers=headers)

        #print(res2.status_code, type(res2.status_code), json_response)

        if res2.status_code not in goodStatusCodes:
            #print("RES-------->", bool(json_response))
            return log_endpoint_no_data(ti, **kwargs)
        
        if payload:
            res = requests.request(rqt, url, headers=headers, json=payload).json()
        else:
        ...
        for x in range (0, totalIterations):
            if currentIterations <= totalIterations:
                if pagination:
                    ...
                    if payload:
                        response = requests.request(rqt, url2, headers=headers, json=payload).json()
                    else:
    ...
    with DAG(dag_id,
        ...
        TokenOffsetTest >> Fetch_Token >> Fetch_Data >> KeyRelationshipS >> Objects_1_2 >> Objects_1_2_SnowFlake >> Arrays_1 >> Arrays_1_SnowFlake >> Arrays_2 >> Arrays_2_SnowFlake >> Arrays_3 >> Arrays_3_SnowFlake >> Arrays_4 >> Arrays_4_SnowFlake >> Log_Endpoint_Complete
...
def all_to_s3():
    ...
    countUrl = vinylDomUrl+"/basictotokenoffset?$count=true&$limit=0"
    headers = {
        'Content-Type': 'application/json',
        'X-API-Key': vinylApiKey
    }
    ...
    limit = 100
    url = vinylDomUrl+"/basictotokenoffset?$count=true&$limit="+str(limit)
    totalIterations = math.ceil(int(count) / int(limit))
    currentIterations = 0
    newOffset = 0
...
with DAG(
    dag_id = 'Dynamic_basicToToken_offset',
